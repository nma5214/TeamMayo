...
SLOTS1 = 10 # number of avaiblable room per day for process 1. Yuli's data, max slot1 = 15
SLOTS2 = 3 * 8 # number of available room per day for process 2. We need to multiply work hour 8. Yuli's data: 2-4 available spots
COLLECTIONS = 5 # the max number of collections a patient needs
NEXT_DAY_P = 0.5 # we want an expectation of 2 patient a day. Consider the geometric distribution, E(x)=1/p=2 then p = 1/2
...
class TwoSchechEnv(gym.Env): #set up the environment
    ...
    def __init__(self):
        ...
        '''Action Space (D1, D2)
        '''
        self.action_space = spaces.MultiDiscrete([DAYS,DAYS]) # each action as (d1,d2)

        '''Internal variables that we need to check the scheduling which is not available to the agent
        '''
    ...
    def reset(self):
        # similar as __init__ ftn but the obs
    ...
    def checkD1(self, d1, collection_num): #returns True if any day during the collectin period is full, else False
